...
void bluDisassembleChunk(bluVM* vm, bluChunk* chunk, const char* name) {
	...
	for (int offset = 0; offset < chunk->count;) {
		offset = bluDisassembleInstruction(vm, chunk, offset);
	}
}

static int constantInstructionN(bluVM* vm, const char* name, int n, bluChunk* chunk, int offset) {
	uint8_t constant = chunk->code[offset + 1];
	printf("%s_%-*d %4d '", name, 15 - (int)strlen(name), n, constant);
	bluPrintValue(vm, chunk->constants.values[constant]);
	printf("'\n");
	return offset + 2;
}

static int constantInstruction(bluVM* vm, const char* name, bluChunk* chunk, int offset) {
	uint8_t constant = chunk->code[offset + 1];
...
static int jumpInstruction(const char* name, bluChunk* chunk, int offset) {
...
int bluDisassembleInstruction(bluVM* vm, bluChunk* chunk, int offset) {
	...
	case OP_INVOKE_6:
	case OP_INVOKE_7:
	case OP_INVOKE_8: return constantInstructionN(vm, "OP_INVOKE_", instruction - OP_INVOKE_0, chunk, offset);
	...
	case OP_SUPER_6:
	case OP_SUPER_7:
	case OP_SUPER_8: return constantInstructionN(vm, "OP_SUPER_", instruction - OP_SUPER_0, chunk, offset);
	...
	case OP_CLOSURE: {
		...
		bluObjFunction* function = AS_FUNCTION(chunk->constants.values[constant]);
